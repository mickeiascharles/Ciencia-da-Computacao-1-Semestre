...
void banho(){
	...
	printf("Nome (pet): ");
	fflush(stdin);
	fgets(agenda_banho[i_banho].nome_pet2, sizeof(agenda_banho[i_banho].nome_pet2), stdin);
...
void consulta(){
	...
	{
		printf(" %d: %s", j + 1, cliente[j].nome);
	}
    
	int cliente_index;
	printf ("Digite a opção: ");
	scanf("%d", &cliente_index);
	agenda_consulta[i_consulta].cliente = cliente[cliente_index - 1];
